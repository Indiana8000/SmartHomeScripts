...
MQTT_TOPIC_L3_CONSUMPTION = "" # L3 Watts, empty -> L1,L2,L3 is calculated
...
def updating_writer(a_context):
    ...
    ep_int1, ep_int2 = calculate_register(float(leistung) * -1)
    ti_int1, ti_int2 = calculate_register(netzbezug_corr)
    exp_int1, exp_int2 = calculate_register(einspeisung_corr)

    if MQTT_TOPIC_L1_CONSUMPTION == "" and MQTT_TOPIC_L2_CONSUMPTION == "" and MQTT_TOPIC_L3_CONSUMPTION == "":
    ...
    l1_int1, l1_int2 = calculate_register(float(l1))
    l2_int1, l2_int2 = calculate_register(float(leistung) * -1)
    l3_int1, l3_int2 = calculate_register(float(l3))
...
###############################################################
class MySlaveContext(ModbusSlaveContext):
